...
Make sure you've started DataStage and apache ( $ datastag-config ), and started "defaultsite" within the config submenu.  If you still have trouble, and are running this with a VM, it's probably easiest to start again from scratch: destroy this VM and create a new one, using default Ubuntu settings.
...
And replace it with < a href=”{% url core:index %>*YourWebsiteNameNoSpaces*< /a >
